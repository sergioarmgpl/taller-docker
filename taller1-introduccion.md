...
## Requisitos
Tener una máquina virtual o instancia en cualquier proveedor de nube como Google Cloud, Azure, Amazon o Digital Ocean, o bien una máquina virtual local. De preferencia con Ubuntu 22.04LTS para estandarizar errores a una distribución y dar soporte en el taller.
...
```
## Paso 2: Bajar imágenes del repositorio de Docker en específico la de Ubuntu 22.04
Para bajar una imagen utilizamos el comando docker pull, en este caso bajaremos la imagen de Ubuntu 22.04 para esto ejecutamos el comando:
```
docker pull ubuntu:22.04
```
Donde ubuntu es el repositorio y el tag a bajar es 22.04, para mayor referencia puedes visitar 
...
Para crear una imagen debes de utilizar el comando docker run, en este caso crearemos un container con Ubuntu 22.04 y ejecutaremos dentro el comando /bin/bash, el cual es una consola o terminal, nótese que al terminar la ejecución del /bin/bash con exit, el comando con el que fue creado el container termina su ejecución y el container es finalizado con el estado exit(0), finalizado más no eliminado
```
